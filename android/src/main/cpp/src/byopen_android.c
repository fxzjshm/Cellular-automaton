...
static by_pointer_t
by_fake_find_maps(by_char_t const *filename, by_char_t *realpath, by_size_t realmaxn) {
    // check
    ...
    if (dynsym && dynstr) {
        for (i = 0; i < dynsym_num; i++, dynsym++) {
            ...
            if ((by_pointer_t) name < end && strcmp(name, symbol) == 0) {
                 ...
                 */
                by_pointer_t symboladdr = (by_pointer_t) (dlctx->baseaddr + dynsym->st_value -
                                                          dlctx->biasaddr);
                by_trace("dlsym(%s): found at .dynsym/%p = %p + %x - %p", symbol, symboladdr,
                         dlctx->baseaddr, (by_int_t) dynsym->st_value, dlctx->biasaddr);
                return symboladdr;
            }
        }
    }
...
static by_fake_dlctx_ref_t by_fake_dlopen(by_char_t const *filename, by_int_t flag) {
    ...
    do {
        ...
        // trace
        by_trace("fake_dlopen: baseaddr: %p, realpath: %s, filesize: %d", baseaddr, realpath,
